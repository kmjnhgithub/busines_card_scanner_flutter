  ...
  group('SettingsPage Widget Tests', () {
    ...
    group('核心功能測試', () {
      ...
      testWidgets('應該顯示設定內容而非載入狀態', (tester) async {
        ...
        // 檢查關鍵的設定類型元件
        final dropdownExists = find.byType(DropdownButton).evaluate().isNotEmpty;
        final segmentedButtonExists = find.byType(SegmentedButton).evaluate().isNotEmpty;
        final switchExists = find.byType(SwitchListTile).evaluate().isNotEmpty;
        
        // 至少應該有一些設定元件
        expect(dropdownExists || segmentedButtonExists || switchExists, isTrue, 
               reason: '應該至少顯示一些設定元件');
      });
    });
    ...
    group('互動功能測試', () {
      testWidgets('應該能夠與語言設定互動', (tester) async {
        ...
        if (dropdowns.evaluate().isNotEmpty) {
          ...
          if (englishOption.evaluate().isNotEmpty) {
            ...
            // Assert - 驗證設定被保存
            verify(() => mockPreferences.setString(any(), any())).called(greaterThanOrEqualTo(1));
          }
        }
      });
      ...
      testWidgets('應該能夠與通知開關互動', (tester) async {
        ...
        if (switches.evaluate().isNotEmpty) {
          ...
          // Assert - 驗證設定被保存
          verify(() => mockPreferences.setBool(any(), any())).called(greaterThanOrEqualTo(1));
        }
      });
    });
    ...
    group('防禦性測試', () {
      testWidgets('應該處理 Provider 異常', (tester) async {
        ...
        when(() => mockPreferences.getString(any())).thenThrow(Exception('Mock error'));
        when(() => mockPreferences.getBool(any())).thenThrow(Exception('Mock error'));
