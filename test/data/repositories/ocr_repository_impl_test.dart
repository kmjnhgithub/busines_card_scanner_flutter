  ...
  group('OCRRepositoryImpl - TDD Red Phase', () {
    ...
    group('recognizeText', () {
      test('should return OCR result when recognition succeeds', () async {
        ...
        when(() => mockCacheService.getCacheKey(testImageData))
            .thenReturn(imageHash);
        when(() => mockCacheService.getCachedResult(imageHash))
            .thenThrow(Exception('Cache miss'));
        when(() => mockOCRService.recognizeText(
          testImageData,
          options: any(named: 'options'),
        )).thenAnswer((_) async => expectedResult);
        when(() => mockCacheService.cacheResult(imageHash, expectedResult))
            .thenAnswer((_) async => {});
        ...
        verify(() => mockOCRService.recognizeText(
          testImageData, 
          options: any(named: 'options')
        )).called(1);
      });
      ...
      test('should use cache when available and valid', () async {
        ...
        when(() => mockCacheService.getCachedResult(imageHash))
            .thenAnswer((_) async => expectedResult);
        when(() => mockCacheService.isCacheValid(expectedResult))
            .thenReturn(true);
        ...
        verifyNever(() => mockOCRService.recognizeText(
          any(), 
          options: any(named: 'options')
        ));
      });
      ...
      test('should fallback to OCR service when cache is invalid', () async {
        ...
        when(() => mockCacheService.isCacheValid(staleResult))
            .thenReturn(false);
        when(() => mockOCRService.recognizeText(
          testImageData, 
          options: any(named: 'options')
        ...
        verify(() => mockCacheService.cacheResult(imageHash, expectedResult)).called(1);
      });
      
      test('should throw OCRProcessingFailure when OCR service fails', () async {
        // Arrange
        const imageHash = 'test_image_hash';
        when(() => mockCacheService.getCacheKey(testImageData))
            .thenReturn(imageHash);
        ...
        when(() => mockOCRService.recognizeText(
          testImageData, 
          options: any(named: 'options')
        )).thenThrow(const OCRProcessingFailure(
          userMessage: 'OCR 引擎處理失敗',
        ));
      ...
      test('should throw UnsupportedImageFormatFailure for invalid image format', () async {
        // Arrange
        final invalidImageData = Uint8List.fromList([0xFF, 0xD8]); // 不完整的 JPEG header
        const imageHash = 'invalid_image_hash';
        
        when(() => mockCacheService.getCacheKey(invalidImageData))
            .thenReturn(imageHash);
        when(() => mockCacheService.getCachedResult(imageHash))
            .thenThrow(Exception('Cache miss'));
        when(() => mockOCRService.recognizeText(
          invalidImageData, 
          options: any(named: 'options')
        )).thenThrow(const UnsupportedImageFormatFailure(
          userMessage: '不支援的圖片格式',
        ));
      ...
      test('should apply OCR options correctly', () async {
        ...
        when(() => mockCacheService.getCachedResult(imageHash))
            .thenThrow(Exception('Cache miss'));
        when(() => mockOCRService.recognizeText(testImageData, options: options))
            .thenAnswer((_) async => expectedResult);
        when(() => mockCacheService.cacheResult(imageHash, expectedResult))
            .thenAnswer((_) async => {});
        
        // Act
        final result = await repository.recognizeText(testImageData, options: options);
        
        // Assert
        expect(result, equals(expectedResult));
        verify(() => mockOCRService.recognizeText(testImageData, options: options)).called(1);
      });
    });
    ...
    group('recognizeTexts (batch processing)', () {
      test('should process multiple images successfully', () async {
        ...
        // 設定快取 mock 行為
        when(() => mockCacheService.getCacheKey(any())).thenAnswer((invocation) {
        ...
        when(() => mockCacheService.getCachedResult(any()))
            .thenThrow(Exception('Cache miss'));
        when(() => mockCacheService.cacheResult(any(), any()))
            .thenAnswer((_) async => {});
    ...
    group('saveOCRResult', () {
      test('should save OCR result successfully', () async {
        ...
        when(() => mockCacheService.saveResult(resultToSave))
      ...
      test('should throw DataSourceFailure when save fails', () async {
        // Arrange
        when(() => mockCacheService.saveResult(any()))
            .thenThrow(const DataSourceFailure(
              userMessage: '儲存失敗',
            ));
    ...
    group('getOCRHistory', () {
      test('should return OCR history with default parameters', () async {
        ...
        when(() => mockCacheService.getHistory())
    ...
    group('deleteOCRResult', () {
      test('should delete OCR result successfully', () async {
        ...
        when(() => mockCacheService.deleteResult(resultId))
    ...
    group('cleanupOldResults', () {
      test('should cleanup old results and return count', () async {
        // Arrange
        when(() => mockCacheService.cleanupOldResults())
    ...
    group('engine management', () {
      test('should return available OCR engines', () async {
        ...
        when(() => mockOCRService.getAvailableEngines())
      ...
      test('should set preferred engine', () async {
        ...
        when(() => mockOCRService.setPreferredEngine(engineId))
            .thenAnswer((_) async => {});
