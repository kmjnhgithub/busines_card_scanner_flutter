...
class GoogleMLKitOCRService implements OCRService {
  ...
  Future<void> _validateImageData(Uint8List imageData) async {
    ...
    if (imageData.isEmpty) {
      throw const UnsupportedImageFormatFailure(userMessage: '圖片資料不能為空');
    }
    ...
    if (imageData.length > _maxImageSizeBytes) {
      throw ImageTooLargeFailure(
        ...
        userMessage:
            '圖片檔案過大 (${(imageData.length / (1024 * 1024)).toStringAsFixed(1)}MB)，最大限制 ${_maxImageSizeBytes / (1024 * 1024)}MB',
      );
    }
    ...
    // 安全性檢查
    final securityResult = _securityService.validateContent(
      String.fromCharCodes(imageData.take(1000)),
    );
    securityResult.fold(
  ...
  Future<Uint8List> _preprocessImageInternal(
    ...
    try {
      ...
      if (image == null) {
        throw const UnsupportedImageFormatFailure(userMessage: '無法解析圖片格式');
      }
      ...
      if (options.sharpen) {
        // 簡化的銳化處理
        processedImage = img.adjustColor(processedImage, contrast: 1.1);
      }
  ...
  Never _throwRepositoryException(Object error, int processingTimeMs) {
    ...
    if (error is TimeoutException) {
      throw OCRServiceUnavailableFailure(
        ...
        userMessage:
            'OCR 處理超時 (${(processingTimeMs / 1000).toStringAsFixed(1)} 秒)',
      );
    }
