...
class EnhancedSecureStorage {
  ...
  Future<Either<DomainFailure, void>> storeApiKey(String service, String apiKey) async {
    try {
      // 驗證服務名稱
      ...
      if (encryptResult.isLeft()) {
        return encryptResult.fold(
          Left.new,
          (_) => const Left(InsufficientPermissionFailure(
    ...
    try {
      ...
      if (servicesResult.isLeft()) {
        return servicesResult.fold(
          Left.new,
          (_) => const Left(DataSourceFailure(
  ...
  Future<Either<DomainFailure, String>> decryptData(String encryptedData) async {
    try {
      ...
      final encryptedContent = combined.sublist(_ivLength, combined.length - _tagLength);
  ...
  DomainValidationFailure? _validateServiceName(String service) {
    ...
    if (!RegExp(r'^[a-zA-Z][a-zA-Z0-9_]*$').hasMatch(service)) {
      return const DomainValidationFailure(
        ...
        internalMessage: 'Service name must start with letter and contain only alphanumeric and underscore',
      );
    }
  ...
  InsufficientPermissionFailure? _checkForMaliciousContent(String input) {
    ...
    // 檢查控制字符
    if (input.contains('\x00') || input.contains('\x01') || input.contains('\x02')) {
      return const InsufficientPermissionFailure(
        permission: 'secure_storage',
        operation: 'input_validation',
        userMessage: 'Invalid content detected',
