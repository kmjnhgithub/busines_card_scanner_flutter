...
class SimpleOCRCacheService implements OCRCacheService {
  ...
  bool isCacheValid(OCRResult result) {
    ...
    final cacheExpiry = result.processedAt.add(const Duration(hours: _cacheValidityHours));
  ...
  Future<int> cleanupOldResults({int daysOld = 30}) async {
    ...
    final oldResults = _history.where((r) => r.processedAt.isBefore(cutoffDate)).toList();
  ...
  Future<OCRStatistics> getStatistics() async {
    ...
    final averageConfidence = totalProcessed > 0 
        ? _history.map((r) => r.confidence).reduce((a, b) => a + b) / totalProcessed
        : 0.0;
    
    final averageProcessingTime = totalProcessed > 0
        ? _history.map((r) => r.processingTimeMs?.toDouble() ?? 0.0).reduce((a, b) => a + b) / totalProcessed
        : 0.0;
