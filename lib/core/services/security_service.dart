...
class SecurityService {
  ...
  static final List<RegExp> _sqlInjectionPatterns = [
    RegExp(r'\b(DROP|DELETE|UNION|INSERT|UPDATE|SELECT)\b', caseSensitive: false),
  ...
  Either<SecurityFailure, String> validateContent(String content) {
    ...
    try {
      ...
      for (final pattern in _maliciousExecutionPatterns) {
        if (pattern.hasMatch(content)) {
          return const Left(SecurityFailure(
            ...
            internalMessage: 'Malicious execution pattern detected in content',
      ...
      // 檢查內容長度（防止 DoS 攻擊）
      if (content.length > 100000) { // 100KB 限制
        return const Left(SecurityFailure(
          userMessage: '內容過長',
      ...
      // 檢查過多的控制字元（排除換行符）
      final controlCharCount = RegExp(r'[\x00-\x08\x0B\x0C\x0E-\x1F\x7F]').allMatches(content).length;
      if (controlCharCount > content.length * 0.2) { // 超過 20% 為控制字元
        return const Left(SecurityFailure(
